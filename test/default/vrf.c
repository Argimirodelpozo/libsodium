...
int main(void)
{
	...
	// ietfdraft03 sizes are unchanged in the final standard
	assert(crypto_vrf_ietfdraft03_SECRETKEYBYTES == crypto_vrf_rfc9381_SECRETKEYBYTES);
    assert(crypto_vrf_ietfdraft03_PUBLICKEYBYTES == crypto_vrf_rfc9381_PUBLICKEYBYTES);
    assert(crypto_vrf_ietfdraft03_SEEDBYTES == crypto_vrf_rfc9381_SEEDBYTES);
    assert(crypto_vrf_ietfdraft03_BYTES == crypto_vrf_rfc9381_BYTES);
    assert(crypto_vrf_ietfdraft03_OUTPUTBYTES == crypto_vrf_rfc9381_OUTPUTBYTES);
	...
	printf("\n");
    for (i = 0U; i < 3U; i++) {
		sodium_hex2bin(seed, 32, test_data[i].seed, (size_t) -1U, NULL, NULL, NULL);

		//keypair generation is exactly the same
		crypto_vrf_seed_keypair(pk, sk, seed);
		printf("%s\n", sodium_bin2hex(pk_hex, sizeof pk_hex, pk, sizeof pk));
		
		if (crypto_vrf_ietfdraft03_prove(proof, messages[i], i, sk) != 0){
			printf("crypto_vrf_prove() error: [%u]\n", i);
		}
		printf("%s\n", sodium_bin2hex(proof_hex, sizeof proof_hex, proof, sizeof proof));
