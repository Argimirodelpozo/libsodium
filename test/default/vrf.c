...
static const TestData2 test_data2[] = {
...
int main(void)
{
    ...
    for (i = 0U; i < (sizeof test_data2) / (sizeof test_data2[0]); i++) {
		...
		// 	continue;
		// }
		if (crypto_vrf_ietfdraft03_prove(proof, (const unsigned char*) test_data2[i].msg, i, sk) != 0){
			printf("crypto_vrf_prove() error: [%u]\n", i);
			continue;
		}
		if (memcmp(test_data2[i].proof, proof, crypto_vrf_PROOFBYTES) != 0){
			printf("proof error: [%u]\n", i);
			printhex("\tWanted: ", test_data2[i].proof, crypto_vrf_PROOFBYTES);
			printhex("\tGot:    ", proof, crypto_vrf_PROOFBYTES);
			continue;
		}
		if (crypto_vrf_ietfdraft03_verify(output, test_data2[i].pk, proof, (const unsigned char*) test_data2[i].msg, i) != 0){
			printf("verify error: [%u]\n", i);
			continue;
		}
		if (memcmp(output, test_data2[i].output, crypto_vrf_OUTPUTBYTES) != 0){
			printf("output wrong: [%u]\n", i);
			printhex("\tWanted: ", test_data2[i].output, crypto_vrf_OUTPUTBYTES);
		...
		if (i > 0) {
			if (crypto_vrf_ietfdraft03_verify(output, test_data2[i].pk, proof, (const unsigned char*) test_data2[i].msg, i-1) == 0){
				printf("verify succeeded with truncated message: [%u]\n", i);
				continue;
			}
		}
